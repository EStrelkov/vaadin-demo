...
public class HotelService extends AbstractTestDataService<Hotel, HotelFilter> {
	...
	@Override
	protected Hotel copy(Hotel hotel) {
		return hotel.clone();
	}

	@Override
	protected boolean isPassesFilter(Hotel hotel, HotelFilter hotelFilter) {
		if (hotelFilter == null) {
			return true;
		}
	...
	@Override
	protected Hotel createTestObject(String[] split, Random r) {
		Hotel h = new Hotel();
		h.setName(split[0]);
		...
		List<HotelCategory> categories = hotelCategoryService.findAll();
		HotelCategory hotelCategory = categories.get(r.nextInt(categories.size()));
		h.setCategoryId(hotelCategory.getId());
		int daysOld = 0 - r.nextInt(365 * 30);
		h.setOperatesFrom((LocalDate.now().plusDays(daysOld).atStartOfDay(ZoneId.systemDefault()).toInstant().toEpochMilli()));
		return h;
	}
